...
async def test_create_policy():
    ...
    weights = ScoringWeights(
        name_vector=0.5, name_trigram=0.2, alias_match=0.1, dob_match=0.1, country_match=0.1
    )
    new_policy = await create_policy(
        mock_session, tenant_id="tenant_1", name="New Version", weights=weights, threshold=0.85
    )
