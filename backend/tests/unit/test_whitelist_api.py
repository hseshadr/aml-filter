...
async def test_trigger_screening_sync():
    ...
    with patch("redis.Redis.from_url", side_effect=Exception("Redis connection failed")):
        ...
        with patch(
            "aml_filter.api.v1.whitelist.BidirectionalScreeningService"
        ) as mock_service_class:
            mock_service_instance = mock_service_class.return_value
            mock_service_instance.screen_whitelist_against_blacklist = AsyncMock(
                return_value={"entities_scanned": 10, "matches_found": 2}
            )
