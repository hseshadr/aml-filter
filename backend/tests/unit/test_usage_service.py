...
from aml_filter.usage.service import get_usage_count, get_usage_summary, record_usage
...
async def test_get_usage_summary():
    ...
    summary = await get_usage_summary(session=mock_session, tenant_id=tenant_id)
...
async def test_get_usage_count():
    ...
    count = await get_usage_count(session=mock_session, tenant_id=tenant_id)
