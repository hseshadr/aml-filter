...
async def test_update_weights_preset():
    ...
    with (
        patch("aml_filter.api.v1.weights.get_active_policy", new_callable=AsyncMock) as mock_get,
...
async def test_get_policy_history():
    ...
    with patch(
        "aml_filter.api.v1.weights.list_policy_versions", new_callable=AsyncMock
    ) as mock_list:
...
async def test_rollback_policy():
    ...
    with patch(
        "aml_filter.api.v1.weights.rollback_to_version", new_callable=AsyncMock
    ) as mock_rollback:
