...
- **`demo.key`** — a 32-byte raw Ed25519 **seed**, copied from
  `backend/examples/keys/trust.key`. Its public half is the committed
  ...
  a real list.
- **`demo_entities.jsonl`** — 8 fabricated `DEMO_SDN` entities (copied from
  `backend/examples/demo_entities.jsonl`). Every name, DOB, and address is invented.
