...
/** Decompressing these bytes would exceed the caller's decompressed byte limit. */
...
/** The bytes are not a single zstd frame that binds its own decompressed size,
 * so no limit can be enforced before decoding. Refused rather than decoded. */
export class UndeclaredSizeError extends Error {
	public constructor() {
		super("zstd input is not a single frame declaring its decompressed size");
		this.name = "UndeclaredSizeError";
	}
}
...
// WASM allocated exactly `outputSize`, so passing `limit + 1` capped expansion.
// 1.15.0 REMOVED that parameter ("Callers must not guess an output size"), and
// JS silently drops extra arguments, so the old call still compiled and ran
// while enforcing nothing. `Zstd.decompress()` is strictly worse: it reads the
// frame's own declared size and allocates that — the attacker-controlled number.
//
...
//      frame's declaration is bypassable: 512 KiB of repeated 147-byte bombs
//      decodes to ~14.9 GB before any output-length check could run.
//   2. That frame DECLARES its Frame_Content_Size, and the declaration is
//      within the caller's limit. libzstd then refuses to emit more content
//      than the frame declared.
//
// Every chunk the producer writes is a single declaring frame, so this rejects
// nothing legitimate. See `decompress`.
 ...
 *
 * Exported for tests: every chunk the producer writes is one declaring frame.
 */
...
/**
 * Decompress bytes that must not expand beyond `limitBytes`.
 *
 * The bound is applied BEFORE the decoder runs: {@link declaredContentSize}
 * refuses anything that does not bindingly declare a size, and a declaration
 * over the limit is rejected, so a tiny frame claiming gigabytes never reaches
 * WASM. The streaming decoder then honours that declaration, `decompressEnd()`
 * rejects a truncated frame, and the output length is re-checked as the final
 * fail-closed gate.
 *
 * `limitBytes` is an inclusive maximum: an output of exactly `limitBytes` is
 * allowed, `limitBytes + 1` is not.
 *
...
export async function decompress(
	...
	const declared = declaredContentSize(bytes);
	if (declared === null) {
		throw new UndeclaredSizeError();
	}
	...
	const plaintext = zstd.decompressChunk(bytes);
	zstd.decompressEnd();
