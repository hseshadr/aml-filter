...
import {
	apiClient,
	type WhitelistCustomerResponse,
	type WhitelistMatchResponse,
} from "../lib/api";
...
export default function WhitelistPage() {
	...
	const [activeTab, setActiveTab] = useState<"customers" | "matches">(
		"customers",
	);
	...
	return (
		<div>
			...
			<div className="tabs">
				...
				>
					Matches (
					{matches.filter((m) => m.resolution_status === "PENDING").length}{" "}
					pending)
				</button>
			</div>
							...
							<tbody>
								{customers.map((customer) => (
									<tr key={customer.entity_id}>
										...
										<td>
											{customer.dob && customer.dob.length > 0
												? customer.dob.join(", ")
												: "-"}
										</td>
										<td>
								...
								>
									<div className="card-header">
										<div>
											...
											{(match.match_score * 100).toFixed(2)}%
										</div>
										<span
											className={getResolutionBadgeClass(
									...
									<div className="text-sm text-muted">
										...
										<div>
											Detected: {new Date(match.detected_at).toLocaleString()}
										</div>
									</div>
								</div>
							))}
