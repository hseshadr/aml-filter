...
interface ImportMetaEnv {
	...
	/** Idle window (ms) for the in-tab ~23 MB model warmup: how long it may go
	 * with no download progress. Fail-closed to the production
	 * `MODEL_LOAD_IDLE_TIMEOUT_MS` (90s) when absent/invalid. Set by the e2e
	 * webServers. */
	 ...
	 * `BOOT_TIMEOUT_MS` (900s, 15 min) default when absent/invalid. Exists only so the
