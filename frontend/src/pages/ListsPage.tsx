...
import { apiClient, ListConfigResponse } from '../lib/api'
            ...
            <div key={list.list_id} className="card flex-between">
              <div className="flex-1">
                ...
                <div className="text-sm text-muted">
                  {list.current_version && <div>Current Version: {list.current_version}</div>}
                  {list.version_override && <div>Override: {list.version_override}</div>}
                  {list.updated_at && <div>Updated: {new Date(list.updated_at).toLocaleString()}</div>}
                </div>
              </div>
              <div>
                <label className="flex flex-gap-sm" style={{ cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={list.enabled}
                    onChange={(e) => handleToggle(list.list_id, e.target.checked)}
                  />
