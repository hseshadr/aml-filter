...
import { apiClient, SearchQuery, SearchResponse, Match, MatchReason } from '../lib/api'
...
export default function SearchPage() {
  ...
  const exportResults = () => {
    ...
    const csv = [
      ['Entity ID', 'Score', 'Risk Category', 'Source List', 'Primary Name', 'Explanation'].join(','),
        ...
        <div className="form-group form-grid">
          <div>
            ...
            <input
              ...
              type="date"
              value={query.dob || ''}
              onChange={(e) => setQuery({ ...query, dob: e.target.value || undefined })}
          ...
          <div>
            ...
            <input
              ...
              type="text"
              value={query.country || ''}
              onChange={(e) => setQuery({ ...query, country: e.target.value || undefined })}
        ...
        <div className="form-group">
          ...
          <select
            ...
            onChange={(e) =>
              setQuery({
                ...query,
                entity_type: (e.target.value as 'PERSON' | 'ORGANIZATION') || undefined,
              })
            }
            ...
            <div className="form-grid">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setQuery({ ...query, threshold: parseFloat(e.target.value) })}
                  style={{ width: '100%' }}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  max="100"
                  value={query.k}
                  onChange={(e) => setQuery({ ...query, k: parseInt(e.target.value) || 20 })}
                  className="form-input"
                />
              </div>
            </div>
      ...
      {results && (
        <div>
          <div className="flex-between mb-md">
            <h2>
              Results ({results.matches.length} matches in {results.execution_time_ms}ms)
            </h2>
            <button onClick={exportResults} className="btn btn-success">
              Export CSV
            </button>
          </div>
            ...
            <div className="card-grid">
              {results.matches.map((match) => (
                ...
                >
                  ...
                  <div className="mt-sm">
                    <span className="badge badge-muted mr-sm">{match.risk_category}</span>
                    <p className="text-sm text-muted mt-sm mb-0">{match.explanation}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {selectedMatch && (
        <div className="modal-overlay" onClick={() => setSelectedMatch(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setSelectedMatch(null)} className="btn btn-danger btn-sm modal-close">
              x
            </button>
            ...
            <div className="mb-md">
              ...
              {selectedMatch.list_version && ` (v${selectedMatch.list_version})`}
            </div>
