...
import org.gainratio.amlfilter.search.utils.Sampling;
...
public class Hierarchy_utils {
    ...
    private static void assignChildrenToParents(VectorSpace pCriteriaVs, VectorSpace pVs, float pSimilarityForTheTraining) {
        ...
        VectorData4Tree child = null;
        VectorData4Tree parent = null;
...
//				pCriteriaVs.getCoordinatesTranslationVectorList().set(i, newVectorTranslator); 
    ...
    public static byte[] getTranslatedCoordinatesRelativeToVs(VectorData4Tree pVectorToTranslate,
    ...
    public static byte[] getTranslatedCoordinatesRelativeToVs(
            VectorData4Tree pVectorToTranslate,
            VectorSpace pCritVs,
...
//		System.out.println("\t\t\t* Sim Norm. = " + retVal);
    ...
    public static void recomputeVectorListCoordinates_RelativeToVs(
            VectorSpace pCritVs,
            List<VectorData4Tree> pListToReposition,
            VsComparisonCriteriaHandler pComparator) throws Exception {

        VectorSpace childVs = null;

        for (int vsPos = 0; vsPos < pListToReposition.size(); vsPos++) {
            VectorData4Tree vectorToReposition = pListToReposition.get(vsPos);
    ...
    public static void recomputeVsCoordinates_RelativeToVs(VectorSpace pCritVs, VectorSpace pVsToReposition) throws Exception {
        ...
        for (int vsPos = 0; vsPos < pVsToReposition.size(); vsPos++) {
            VectorData4Tree vectorToReposition = pVsToReposition.get(vsPos);
...
//					// Clear the mark to allow exit without the mark if not trained
                    ...
                    VectorData4Tree criteriaVector = pReferenceVs.get(i);
                    ...
                    for (int resultsPosition = 0; resultsPosition < similarResultsInVs.size(); resultsPosition++) {
                        ...
                        VectorData4Tree possibleNewCenter = similarResultsInVs.get(resultsPosition).getFoundVectorData();
                        ...
                        if (!APPLY_MARKED_VECTOR_EXCLUSIONS || (APPLY_MARKED_VECTOR_EXCLUSIONS && possibleNewCenter.isMarked())) {
                            ...
                            if (avg > maxDensity) {
                                ...
                                VectorData4Tree newCenter = possibleNewCenter.clone();
    ...
    public static void testAllChildrenPointToParent(VectorData4Tree pParent) throws Exception {
...
//
    ...
    private boolean addVectorcitoToTree_old(
        ...
        VectorData4Tree mostSimilarParent = null;
...
//			Choose the most similar one
    ...
    private boolean decideDescent(
            VectorData4Tree pVectorToAdd,
            VectorData4Tree pSimilarParent,
            VectorSpace pOrderedVs,
     ...
     */
    public List<VectorData4Tree> addNewVectorsToTree(
            List<VectorData4Tree> pVectorsToAdd,
...
//					pRawVs.addVector(pVectorsToAdd.get(i));
    ...
    public void adjustDistanceToParent_ofChildren(VectorData4Tree pParentVector) throws Exception {
    ...
    public boolean checkCoordinateSizeUpOnTheTree_comparingWithParents(VectorSpace pVsToCheck) throws Exception {
        ...
        VectorData4Tree lastParent = null;
    ...
    public List<byte[]> createSeedingByteArrayListFromVs(
            VectorSpace pVs,
            List<VectorData4Tree> pDistantVectors
    ) throws Exception {
...
//		separatedVectors = eliminateSimilarVectors( separatedVectors );
    ...
    private List<VectorData4Tree> separateVectors(List<VectorData4Tree> pVectorsToSeparate, VectorSpace pRawVs) {


        return pVectorsToSeparate;
    }

    private List<VectorData4Tree> eliminateSimilarVectors(List<VectorData4Tree> pVectorsToSeparate) {

        return pVectorsToSeparate;
    }

    private void hangVectorFromParentUsingResult(
            VectorData4Tree pVectorToHang,
        ...
        VectorData4Tree parentVector = pParentResult.getFoundVectorData();
    ...
    public void increaseMaxDistanceToChildren_onParentPathToTop(VectorData4Tree pVectorToHang) throws Exception {

        VectorData4Tree lastParent = pVectorToHang.getParentVector();
...
//		}
    ...
    public void increaseMaxDistanceToChildren_onParentPathToBottom_respectToThisParent(
            VectorData4Tree pParentToAdjust,
            VectorSpace pVsToMonitor) throws Exception {

        for (int i = 0; i < pVsToMonitor.size(); i++) {
            VectorData4Tree child_vector = pVsToMonitor.get(i);
    ...
    public void recomputeChildrenDistancesToParents(VectorSpace pRawVs, VectorSpace pOrderedVs) throws Exception {
        VectorData4Tree childVector = null;
    ...
    public void recomputeChildDistanceToParent(VectorData4Tree pVector) throws Exception {
        VectorData4Tree parent = pVector.getParentVector();
...
//		logLine(log,line);
    ...
    public void show_results(BufferedWriter log, List<TreeResult> results) throws Exception {
        for (int resPos = 0; resPos < results.size(); resPos++) {
            VectorData4Tree vectorData = results.get(resPos).getFoundVectorData();
    ...
    public void show_results_with_original_similarities(
            ...
            VectorSpace pVs,
            VectorData4Tree pSearchedVector) throws Exception {
        ...
        for (int resPos = 0; resPos < results.size(); resPos++) {
            ...
            VectorData4Tree v1 = resultado.getFoundVectorData();
            VectorData4Tree v2 = pSearchedVector;
    ...
    public void show_vdList(List<VectorData4Tree> pVectorList) throws Exception {
...
//		similarityForTheTraining = (float)pCriteriaVs.getComparator().getHalfWayToMinimumSimilarity( similarityForTheTraining );
        ...
        if (pRelocateCoordinates_relativeToParents) {
            ...
            // --------------------------------------------------------------------------
            if (processOrphans) {
                List<VectorData4Tree> remainingOrphans = addVectorsToTree(
                        pCriteriaVs.getOrphanList(),
                        pCriteriaVs,
                        pRawVs);
//
...
//				// Set the remaining orphans in the vs
            ...
            VectorData4Tree firstTroublesomeVector = pRawVs.findParentWithZeroMaxDistDrillDown();
