...
//			Hierarchy_utils.logLine(Hierarchy_utils.log,"\t# Training: " + trainingTime + " min");
            ...
            VectorData4Tree v2s = readVs.createVector("ROBERT GABRIEL MUGABE", readVs.getOriginalComparatorWhenTraining());
...
//			thresholdAdjustmentTest(target, orderedVs, rawVs, hu, searchDistance);
    ...
    private static VectorData4Tree test_tree_search_batch(
        ...
        for (int i = 0; i < pRawVs.size(); i++) {
            ...
            VectorData4Tree vectorToSearchTranslated = null;
            boolean found = false;
        ...
        VectorData4Tree vector2Search = pTrainedVs.createVector(
                name,
                pTrainedVs.getOriginalComparatorWhenTraining());
        System.out.println();
