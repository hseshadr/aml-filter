...
public class Sampling {
    ...
    public final static List<VectorData> buildRandomSample(VectorSpace pVectorSpace,
                                                           int pSampleSize,
...
//			throw new Exception("buildRandomSample -> the parameters could take a long time. Exiting...(" + pSampleSize + " / " + pVectorSpace.size() + ")");
            if (pCloneItemsToReturnNewElements) {
                ...
                for (int i = 0; i < pVectorSpace.size(); i++) {
                    VectorData newVector = pVectorSpace.get(i).clone();
...
//				pVectorSpace.get(randomPos).unsetMark();
    ...
    public final static VectorData chooseRandomVector(List<VectorData> pListOfVectors, boolean pSkipMarkedVectors) throws Exception {
    ...
    private final static int getNextNotMarkedVector(List<VectorData> pListOfVectors, int pOriginPos) throws Exception {
        int pos = pOriginPos;
    ...
    public static List<VectorData> retrieveClonedSignificantVectorsFromVs(
            VectorSpace pRawVs,
            double pAvgDistanceRawSpace,
            float pPercentOfSpaceToCover) throws Exception {

        List<VectorData> retList = new ArrayList<VectorData>();
        float percentageOfSpaceCovered = 0f;
        int numVectorsCovered = 0;
        VectorData temptativeVector = null;
